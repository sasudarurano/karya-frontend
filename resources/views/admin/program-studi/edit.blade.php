...
<div class="max-w-3xl mx-auto pb-20 px-4 sm:px-6">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Program Studi</h1>
    ...
    <form action="{{ route('admin.program-studi.update', $programStudi['id']) }}" method="POST" class="space-y-6">
            ...
            <label for="code" class="block text-sm font-semibold text-gray-900 mb-2">
                Kode Program Studi <span class="text-red-500">*</span>
                 ...
                 <input type="text" id="code" name="code" required
                     value="{{ old('code', $programStudi['code'] ?? '') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <input type="text" id="faculty" name="faculty" required
                   value="{{ old('faculty', $programStudi['faculty'] ?? '') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <textarea id="description" name="description" rows="4"
                      ...
                      placeholder="Deskripsi singkat tentang program studi...">{{ old('description', $programStudi['description'] ?? '') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="bg-gray-50 rounded-xl border border-gray-200 p-6">
            ...
            <div class="space-y-2 text-sm">
                @if(isset($programStudi['created_at']))
                <div class="flex justify-between">
                    <span class="text-gray-600">Dibuat:</span>
                    <span class="text-gray-900 font-medium">{{ \Carbon\Carbon::parse($programStudi['created_at'])->format('d F Y, H:i') }}</span>
                </div>
                @endif
                @if(isset($programStudi['updated_at']))
                <div class="flex justify-between">
                    <span class="text-gray-600">Terakhir diubah:</span>
                    <span class="text-gray-900 font-medium">{{ \Carbon\Carbon::parse($programStudi['updated_at'])->format('d F Y, H:i') }}</span>
                </div>
                @endif
            </div>
        </div>
